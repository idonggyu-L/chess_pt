...
_ALL_SQUARES_MASK = np.iinfo(np.uint64).max


def _get_pieces(state, color: chess.Color):
    is_player_black = color == chess.BLACK
    white_pieces = state["white_pieces"]
    black_pieces = state["black_pieces"]
    if is_player_black:
        pieces_planes = black_pieces + white_pieces
        return tuple(chess.flip_vertical(p) for p in pieces_planes)
    else:
        return white_pieces + black_pieces


def _get_castlings(state, color: chess.Color):
    if color == chess.WHITE:
        return (
            state["white_queenside_castling"],
            state["white_kingside_castling"],
            state["black_queenside_castling"],
            state["black_kingside_castling"],
        )
    else:
        return (
            state["black_queenside_castling"],
...
def stack_observations(history, history_size=8):
    """Stack history."""
    last_position = history[-1]
    us = last_position["turn"]
    planes = []
    for position in reversed(history):
        # Get history pieces position from the perspective of the current player
        history_planes = _get_pieces(position, us)
        for plane in history_planes:
            planes.append(plane)
        if position["is_repetition"]:
            planes.append(_ALL_SQUARES_MASK)
        else:
            planes.append(np.uint64(0))
        if len(planes) >= 13 * history_size:
            break

    while len(planes) < 13 * history_size:
        planes.append(np.uint64(0))
    ...
    (
        ...
        castling_them_ooo,
        castling_them_oo,
    ) = _get_castlings(last_position, us)

    us_black = us == chess.BLACK
    rule50_counter = last_position["halfmove_clock"]
    # Move count is zeroed out
    move_count = last_position["fullmove_number"]
    planes = np.frombuffer(
        np.asarray(planes, dtype=np.uint64).tobytes(), dtype=np.uint8
    )
    return {
        "planes": planes,
        "us_black": us_black,
        "move_count": move_count,
        "rule50_counter": rule50_counter,
...
def get_board_features(board: chess.Board):
    """Get features from the current chess board."""
    turn = board.turn
    ...
    white_kingside_castling = board.has_kingside_castling_rights(chess.WHITE)
    white_queenside_castling = board.has_queenside_castling_rights(chess.WHITE)
    black_kingside_castling = board.has_kingside_castling_rights(chess.BLACK)
    black_queenside_castling = board.has_queenside_castling_rights(chess.BLACK)
    return {
        "turn": turn,
        ...
        "white_kingside_castling": white_kingside_castling,
        "white_queenside_castling": white_queenside_castling,
        "black_kingside_castling": black_kingside_castling,
        "black_queenside_castling": black_queenside_castling,
    }


def get_lc0_input_planes(observation):
    """Convert to input representation used by LC0"""
    history_planes = (
        np.unpackbits(
            np.frombuffer(observation["planes"].tobytes(), dtype=np.uint8),
            # NOTE: this is a different bitorder used in TrainingData format
            # as the bits in a byte are flipped in TrainingData. See
            # https://github.com/LeelaChessZero/lc0/blob/master/src/trainingdata/trainingdata.cc#L132
            bitorder="little",
        )
        .reshape((104, 8, 8))
        .astype(np.float32)
    ...
    castling_us_ooo = binary_planes[observation["castling_us_ooo"]]
    castling_us_oo = binary_planes[observation["castling_us_oo"]]
    castling_them_ooo = binary_planes[observation["castling_them_ooo"]]
    castling_them_oo = binary_planes[observation["castling_them_oo"]]
    side_to_move_plane = binary_planes[observation["us_black"]]

    rule50_plane = np.full(
        (1, 8, 8), observation["rule50_counter"] / 99.0, dtype=np.float32
    )

    return np.concatenate(
        [
            # Stacked history planes of pieces and repetitions
            history_planes,  # 0-103
            # castling planes
            castling_us_ooo,  # 104
            castling_us_oo,  # 105
            castling_them_ooo,  # 106
            castling_them_oo,  # 107
            # color planes
            side_to_move_plane,  # 108
            # rule50 plane
            rule50_plane,  # 109
            # zeroed out move count plane
            binary_planes[0],  # 110
            # all-ones plane
            binary_planes[1],  # 111
        ]
...
def _unpackplanes(planes):
    planes = planes[..., None]
    bits = tf.constant([1, 2, 4, 8, 16, 32, 64, 128], dtype=tf.uint8)
    return tf.cast(tf.reshape(tf.bitwise.bitwise_and(planes, bits) > 0, [-1]), tf.uint8)


@tf.function
def get_lc0_input_planes_tf(observation):
    is_batched = True
    if len(observation["us_black"].shape) == 0:
        observation = tf.nest.map_structure(
            lambda x: tf.expand_dims(x, axis=0), observation
        )
        is_batched = False
    if observation["us_black"].shape[:1].is_fully_defined():
        batch_size = observation["us_black"].shape[0]
    else:
        batch_size = tf.shape(observation["us_black"])[0]
    history_planes = tf.cast(
        tf.reshape(_unpackplanes(observation["planes"]), (batch_size, 104, 8, 8)),
        tf.float32,
    )
    # ones_plane = tf.ones((1, 8, 8), dtype=tf.float32)
    castling_us_ooo = tf.cast(observation["castling_us_ooo"], tf.float32)
    castling_us_oo = tf.cast(observation["castling_us_oo"], tf.float32)
    castling_them_ooo = tf.cast(observation["castling_them_ooo"], tf.float32)
    castling_them_oo = tf.cast(observation["castling_them_oo"], tf.float32)
    side_to_move_plane = tf.cast(observation["us_black"], tf.float32)
    rule50_plane = tf.cast(observation["rule50_counter"], tf.float32) / 99.0

    # pylint: disable=all
    state_planes = tf.stack(
        [
            # castling planes
            castling_us_ooo,  # 104
            castling_us_oo,  # 105
            ...
            # rule50 plane
            rule50_plane,
            tf.zeros_like(side_to_move_plane, dtype=tf.float32),
            tf.ones_like(side_to_move_plane, dtype=tf.float32),
        ],
        axis=-1,
    )
    state_planes = tf.expand_dims(tf.expand_dims(state_planes, -1), -1) * tf.ones(
        (8, 8), dtype=tf.float32
    )
    output = tf.concat(
        [
            # Stacked history planes of pieces and repetitions
...
# Copied from lczero-training
# https://github.com/LeelaChessZero/lczero-training/blob/44fc71f8f03a00b11431fbc3d937f071317ac3db/tf/policy_index.py
# fmt: off
LC0_POLICY_INDEX = [
...
MOVE_TO_INDEX_MAP = {uci_move: i for i, uci_move in enumerate(LC0_POLICY_INDEX)}

def as_lc0_policy_index(move: str) -> int:
    """Convert a move to the action index used in LC0.
    Note:
        the move must be from the perspective of the current player. Therefore,
        the black player's move needs to be flipped before converting to the policy
        index.
    """
    if move.endswith("n"):
        move = move[:-1]
...
if __name__ == '__main__':
    with open(os.path.join('/home/hail/ChessGPT/chess_ai/lichess_db_standard_rated_2013-02.pgn'), "rt") as f:
            pgn = f.read()
    ...
    for t, move in enumerate(moves):
        ...
        input_planes = get_lc0_input_planes(
